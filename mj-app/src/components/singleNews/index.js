...
class singlenews extends Component {
  ...
  allComments() {
    newsData.allComments().then((result) => {
      console.log(result.data.objects);
      var allData = result.data.objects;
      if (allData) {
  ...
  submitComment = (e) => {
    ...
    newsData
      .submitComment(this.state.name, this.state.comment, this.state.newsId)
      .then((result) => {
        console.log(result);
  ...
  render() {
    return (
      <div>
        ...
        <h1 className="blog-title">{this.state.singleNewsData.title}</h1>

        <div>{ReactHtmlParser(this.state.singleNewsData.content)}</div>
